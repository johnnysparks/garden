...
export function createTurnManager(): TurnManager {
  ...
  function advancePhase(): void {
    ...
    if (current === TurnPhase.ADVANCE) {
      ...
      // Energy is recalculated at the PLAN→ACT transition via beginWork().
      // Resetting here ensures the display during DAWN/PLAN phases shows
      // "0/0" rather than a stale value from the previous week.
      week.update((w) => w + 1);
